...
def update_customer(CustomerId, Name=None, LastName=None, Email=None):
...
# conn = psycopg2.connect(database="customer_db", user="postgres", password="583410")
# with conn.cursor() as cur:
with psycopg2.connect(database="customer_db", user="postgres", password="583410") as conn:
    with conn.cursor() as cur:
        # find_customer(LastName='Mustang')
        # delete_customer(10)
        # delete_phone(4)
        # create_tables()
        # add_new_customer(cur, 'Наталья', 'Коробкова', 'коробок@yahoo.com', '+7(921)420-34-15')
        # add_phone(5, '+7(962)388-35-99')
        update_customer(5, 'Андрей', 'Чуров', 'sima@noll.rom')
